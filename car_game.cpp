...
int enemyPostionX; // used to initialize the X-coordinate
...

{
    ...
    // giving game dimension of the circular-effect
    height = 20;
    width = 19;

    //  Setting the starting position of the car.
    myCarX = 8;
    myCarY = 16;
...
void layout() // to print the game layout.
{
    ...
    gotoxy(5, 21); // gotoxy(coloums,rows)
    cout << "Life:" << life;
...
void drawMyCar() // "The drawMyCar function is responsible for performing the player's car on the console screen."
{
    ...
    if (!dead)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            cout << myCar[i]; // prints the car's parts according to the array
        }
...
{
    if (_kbhit()) // use to determine if a keyboard key is pressed. This function is part of the <conio.h>
                  // doesn't requires enter after pressing a key.
                  // header and returns a non-zero value if a key is pressed."
    {
        switch (getch()) // getch() is a function from <conio.h> used to read single charecter.
...
{
    srand(time(NULL)); // The function is used to generate the random number with the current time to ensure different random values on each run."

    if (isMoving) /// change the game state between moving and not moving
    ...
    // If a collision is happens, it changes the game state to show the player is dead.
    for (int i = 0; i < enemyNum; i++)
    {
        if ((myCarY <= enemyY[i] + 3 && myCarY >= enemyY[i] || enemyY[i] >= myCarY && enemyY[i] <= myCarY + 3) && myCarX == enemyX[i])
        // This above code helps in determining whether a collision has occurred between the player's car and any of the enemy cars.
...
If a collision is detected, it changes the dead variable to true, showing that the player has lost the game.*/
        {
            // Change the game state to show the player is dead
            dead = true;
        }
    }
}
...
void bombExplosion() // to print bomb explosion effect
{
    ...
    Sleep(100); // Give a delay for a goood visual effect
}
//------------------------------------------------------------------------------
